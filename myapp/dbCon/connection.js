...
function addData(req, res) {
    ...
    const sql = `INSERT INTO expenses (userID, categoryID, amount, date, notes) VALUES (?, 1, ?, ?, ?)`;
...
function addInvest(req,res){
    ...
    const sql = `INSERT INTO investments (userID,categoryID,investmentName, amount, purchaseDate,currentValue) VALUES (?, 3,?,?,?,?)`;
    con.query(sql, [userId,investmentName, amount, purchaseDate,currentValue], (err, result) => {
        if (err){
            throw err;
        }else {
...
function addSub(req,res){
    ...
    const sql = `INSERT INTO subscriptions (userID,categoryID,subscriptionName, amount, startDate, endDate, renewalPeriod, reminderOn, notes) 
                     VALUES ( ?, 6, ?, ?, ?, ?,?,?,?)`;
    
    con.query(sql, [userId,subscriptionName, amount, startDate, endDate, renewalPeriod, reminderOn, note], (err, result) => {
            if (err) {
